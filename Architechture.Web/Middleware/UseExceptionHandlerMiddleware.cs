...
using Architecture.Core.Services.HttpRequestRead;
...
namespace Architechture.Web.Middleware
{
    public class UseExceptionHandlerMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            try
            {
                requestBodyData = await context.Request.GetRawBodyAsync();
                await _next(context);
            }

            // This catch is used for when user cancel task from UI.
            ...
            catch (Exception error)
            {
                ...
                if (Convert.ToString(error.GetType()) != typeof(SystemInternalException).ToString())
                {
        ...
        private async Task SendEmailAsync(dynamic logEntry)
        {
            ...
            var applicationEnvironment = Convert.ToString(_configuration["AppSettings:ApplicationEnvironment"]);
            ...
            if (sendExceptionEmail == "1" && exceptionEmailToList.Any())
            {
                ...
                {
                    await _emailHelper.SendEmail(subject: "Error Exception | TANYO-PORTAL | " + applicationEnvironment + " | " + DateTime.Now, htmlContent: htmlContent, to: exceptionEmailToList);
                }
            }
        }
        ...
        private static string GenerateHTMl(dynamic logEntry)
        {
            ...
            string htmlContent = htmlTableStart + htmlTrStart + htmlTdStart + "RequestPath" + htmlTdEnd + htmlTdStart + logEntry.RequestPath + htmlTdEnd + htmlTrEnd
                                 ...
                                 + htmlTrStart + htmlTdStart + "RequestBody" + htmlTdEnd + htmlTdStart + logEntry.RequestBody + htmlTdEnd + htmlTrEnd
