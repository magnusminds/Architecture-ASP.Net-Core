...
namespace Architecture.BusinessLogic.Repositories
{
    public class UsersBL : IUsersBL
    {
        ...
        private readonly IUserRolesDA _userRole;
        private readonly IRoleDA _role;

        public UsersBL(IUserDA user, IUserRolesDA userRole, IRoleDA role)
        {
            _user = user;
            _userRole = userRole;
            _role = role;
        ...
        public LoginViewModelResponse Validateuser(string userName, string password)
        {
            ...
            var UserInfo = (from u in _user.GetUsers()
                            ...
                            select new LoginViewModelResponse()
                            {
                                ...
                                RoleId = Enumerable.Empty<string>(),
                                RoleName = Enumerable.Empty<string>()
                            }).FirstOrDefault();
            return UserInfo;
        }
